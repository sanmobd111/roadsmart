...
export default function ApplyFinanceStep({
  data,
  setData,
  handleNext,
  handlePrev,
  ...
  // Form state
  const [vehicleName1, setVehicleName1] = useState(data?.manufacturer || "");
  const [vehicleName2, setVehicleName2] = useState(data?.model || "");
  const [currency, setCurrency] = useState(data?.currency || "USD");
  const [downPayment, setDownPayment] = useState(data?.downPayment || "");
  const [uploadedFiles, setUploadedFiles] = useState(
    data?.uploadedFiles || Array(5).fill(null)
  );
      ...
      <div className="bg-white">
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          ...
          <h2 className="text-xl md:text-2xl lg:text-3xl font-semibold text-gray-secondary mb-6 text-center lg:text-nowrap relative pl-10 lg:pl-0">
            <BackButton
              className={"lg:-translate-x-[10vw] top-0 left-4"}
              onclick={handlePrev}
            />
            Apply for finance
          </h2>
